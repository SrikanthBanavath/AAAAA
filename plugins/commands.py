...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await message.reply(script.START_TXT_GROUP.format(message.from_user.mention if message.from_user else message.chat.title, temp.U_NAME, temp.B_NAME), reply_markup=reply_markup, disable_web_page_preview=True)
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAEMi-Jmo14cdjGVVTNnVimPhV4rOXUnvQACmgADyJRkFCxl4eFc7yVqNQQ") 
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url="https://t.me/CinemasMawa_Ott")
                    ],[
                        InlineKeyboardButton("🖥𝗪𝗮𝘁𝗰𝗵 𝗢𝗻𝗹𝗶𝗻𝗲", url=stream),  # we download Link
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    InlineKeyboardButton("Updates Channel", url="t.me/CinemasMawa_OTT")
                ],[
                    InlineKeyboardButton('🖥Watch Online / Fast Download⚡️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @KingVJ01
                ]]
            else:
                button = [[
                    InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url=f'https://t.me/{SUPPORT_CHAT}'),
    ...
    if not files_:
        ...
        try:
            ...
            title = '@CinemasMawa  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@CinemasMawa  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@CinemasMawa  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
